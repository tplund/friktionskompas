...
# ========================================
# PAIR ROUTES (par-måling) - MUST be before /profil/<session_id>
# ========================================
...
@app.route('/api/debug/test-submit/<session_id>')
def debug_test_submit(session_id):
    """Debug endpoint til at teste submit-processen trin for trin"""
    ...
    try:
        # Step 1: Get session
        results["steps"].append({"step": "get_profil_session", "status": "starting"})
        profil_session = get_profil_session(session_id)
        if not profil_session:
            results["steps"][-1]["status"] = "FAILED - session not found"
            return jsonify(results)
        results["steps"][-1]["status"] = "OK"
        results["steps"][-1]["session_data"] = {
            "id": profil_session.get("id"),
            "is_complete": profil_session.get("is_complete"),
            "context": profil_session.get("context")
        }

        # Step 2: Get pair session
        results["steps"].append({"step": "get_pair_session_by_profil_session", "status": "starting"})
        pair = get_pair_session_by_profil_session(session_id)
        results["steps"][-1]["status"] = "OK"
        results["steps"][-1]["has_pair"] = pair is not None
        if pair:
            results["steps"][-1]["pair_data"] = {
                "id": pair.get("id"),
                "pair_mode": pair.get("pair_mode"),
                "status": pair.get("status")
            }

        # Step 3: Simulate parsing
        pair_mode = (pair.get('pair_mode') or 'standard') if pair else 'basis'
        results["steps"].append({"step": "determine_pair_mode", "pair_mode": pair_mode})

        # Step 4: Test save_responses
        results["steps"].append({"step": "save_profil_responses", "status": "starting"})
        test_responses = {1: 5}  # Just one test response
        try:
            save_profil_responses(session_id, test_responses, response_type='own')
            results["steps"][-1]["status"] = "OK"
        except Exception as e:
            results["steps"][-1]["status"] = f"FAILED: {str(e)}"
            return jsonify(results)

        # Step 5: Test complete_session (but don't actually complete)
        results["steps"].append({"step": "complete_profil_session", "status": "SKIPPED (would mark complete)"})

        # Step 6: Test update_pair_status
        if pair:
            results["steps"].append({"step": "update_pair_status", "status": "starting"})
            try:
                status = update_pair_status(pair['id'])
                results["steps"][-1]["status"] = f"OK - status={status}"
            except Exception as e:
                results["steps"][-1]["status"] = f"FAILED: {str(e)}"
